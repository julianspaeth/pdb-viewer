...
public class CommandManager {
    ...
    public CommandManager() {
        undoStack.addListener((InvalidationListener) (e) -> {
            undoName.set(undoStack.size() > 0 ? "Undo " + undoStack.get(undoStack.size() - 1).getName() : "Undo");
        });
        redoStack.addListener((InvalidationListener) (e) -> {
            redoName.set(redoStack.size() > 0 ? "Redo " + redoStack.get(redoStack.size() - 1).getName() : "Redo");
        });
    }
    ...
    public ObservableList<ACommand> getUndoStack() {
        return undoStack;
    }

    public ObservableList<ACommand> getRedoStack() {
        return redoStack;
    }
}
